...
def clf_loop(models_to_run, clfs, grid, X, y, grid_size):
            ...
            for p in ParameterGrid(parameter_values):
                try:
                    ...
                    results_df.loc[len(results_df)] = [models_to_run[index],clf, p,
                        ...
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,1.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,2.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,5.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,10.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,20.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,30.0),
                        ut.accuracy_at_k(base_y_test_sorted,baseline_y_pred_probs_sorted,50.0),
                    ...
                    name = './graphs/p_r_graph'+ models_to_run[index] + '-{}-'.format(grid_size) + datetime.now().strftime("%m-%d-%Y, %H:%M:%S")
                    ut.plot_precision_recall_n(y_test, y_pred_probs, name, 'save')
                    roc_name = './graphs/roc_graph'+ models_to_run[index] + '-{}-'.format(grid_size) + datetime.now().strftime("%m-%d-%Y, %H:%M:%S")
